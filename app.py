...
if image_file is not None:
    ...
    with col1:
        ...
        if options:
            ...
            for op in options:
                if op == 'grayscale':
                    if  not opration_flag:
                        processimage = cv2.cvtColor(np.array(image),cv2.COLOR_RGB2GRAY)
                        processimage = cv2.cvtColor(processimage , cv2.COLOR_GRAY2RGB)
                    else:
                        processimage = cv2.cvtColor(processimage , cv2.COLOR_GRAY2RGB)
                        processimage = cv2.cvtColor(np.array(processimage),cv2.COLOR_RGB2GRAY)
                    opration_flag = True
                if op == 'Brightness':
                    # st.write('Select Brightness factor')
                    brightness_factor = st.slider('Brightness', min_value=0, max_value= 100, value= 20)
                    if not opration_flag:
                        fl_image = np.array(image).astype('float')
                        ...
                        fl_image = np.array(processimage).astype('float')
                        adjusted_image = fl_image * brightness_factor/20
                ...
                if op == 'Blur':
                    ...
                    if  not opration_flag:
                        processimage = cv2.blur(np.array(image) , ksize=(k,k))
                    else:
                        processimage = cv2.blur(np.array(processimage) , ksize=(k,k))
                    opration_flag = True
                if op == 'Edge Detection':
                    ...
                    if  not opration_flag:
                        processimage = cv2.Canny(np.array(image) , threshold1=th_lower , threshold2= th_upper)
                    else:
                        processimage = cv2.Canny(np.array(processimage) , threshold1=th_lower , threshold2= th_upper)
                    opration_flag = True
            ...
            for t in transformation:
                if t== 'Rotation':
                    ...
                    if not transform_flag:
                        ...
                        transform_image = cv2.warpAffine(np.array(image),M,(cols,rows)) 
                    if opration_flag :
                        transform_image = cv2.warpAffine(np.array(processimage),M,(cols,rows)) 
                ...
                if t== 'Resize':
                    ...
                    if not transform_flag:
                        transform_image = cv2.resize(np.array(image) , (cols*resize_facor//100 ,rows*resize_facor//100 ) , interpolation=cv2.INTER_AREA)
                    if opration_flag :
                        transform_image = cv2.resize(np.array(processimage) , (cols*resize_facor//100 ,rows*resize_facor//100 ))
                ...
                if t== 'Crop':
                    ...
                    if not transform_flag:
                        transform_image = np.array(image)[X1:X1+height , X2:X2+width]
                    else:
                        transform_image = np.array(processimage)
